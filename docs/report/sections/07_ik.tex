...
singular configurations. The weighting does one specific thing: the arm has five
joints and the task has six degrees of freedom, so exactly \emph{one} direction
in task space is unreachable, and the small entry de-weights that one attitude
component so the solver spends its five degrees of freedom on position first.
It is not a general de-emphasis of orientation.
