...
Proximity  & Injection error                    & $\mDrZero\unit{m}$, $\mDvZero\unit{m/s}$ \\
           ...
           & \quad Monte Carlo, $\mMCn$ draws, P05 / P50 / P95 & $\mMCpFive$ / $\mMCpFifty$ / $\mMCpNinetyFive\unit{m/s}$ \\
...
Perturbed  & $J_2$ miss, no retargeting         & $\mMissJtwo\unit{km}$ \\
           ...
           & Mid-course retarget, mid-mission   & $\mMccReal\unit{m/s}$ \\
           & \quad naive CW estimate, for contrast & $\mMccNaive\unit{m/s}$ \\
           & $J_2$ secular rate vs Vallado      & $\mJtwoRateRel$ rel. \\
           & Finite-burn penalty, $T/W = 0.1$   & $\mGlossA\unit{m/s}$ \\
\midrule
CR3BP      & Converged miss                     & $\mCRmiss\unit{m}$ \\
           & Correction to the two-body design  & $\mCRsigned\unit{mm/s}$ \\
           & \quad family spread over $\pm40\unit{s}$ of $t_f$ & $\mFamSpread\unit{mm/s}$ \\
\midrule
...
the identical code path closes to $\mMissTwoBody\unit{m}$. None is at fault.
...
\paragraph{CR3BP correction: a family, not a discrepancy.}
The planar shooting problem is rank-deficient by one, so the zero-miss set is a
curve and different optimiser starts land at different points on it. Rather than
defend one point, Section~\ref{sec:cr3bp} now publishes the family: scanning
$t_f$ over $\pm40\unit{s}$ and re-optimising gives \mFamN{} members that all
close below one metre, spanning $\mFamSpread\unit{mm/s}$ of budget. The
independently reported member near $\Delta t_f = +17\unit{s}$ sits on that same
curve at $\mFamRefDV\unit{mm/s}$. There is no disagreement to reconcile, and
forcing one optimiser to land on another's point would have been curve-fitting.
...
\item \textbf{The arm is kinematic and static.} Five degrees of freedom, a base
      ...
      during berthing.
\item \textbf{The error law is assumed, even though the spread is now sampled.}
      The headline proximity numbers come from one draw, but $\mMCn$ further
      draws of the same law give P05/P50/P95 of
      $\mMCpFive$/$\mMCpFifty$/$\mMCpNinetyFive\unit{m/s}$ for the hold, so the
      official figure is no longer a single unqualified sample. What remains
      unjustified is the law itself: a uniform magnitude with an isotropic
      direction is a placeholder for a real ascent-guidance covariance.
\end{itemize}

\subsection{Four things a reader should not take from this report}

The limitations above are model boundaries. These four are calibration
statements: they say how far the numbers are from an actual programme.

\begin{itemize}
\item \textbf{The Artemis architecture is not this one.} Human Landing System
      rendezvous is with a Gateway or Orion stack on a near-rectilinear halo
      orbit, not a $400\unit{km}$ circular LLO. The Hohmann-plus-phasing
      arithmetic of Section~\ref{sec:transfer} does not carry over to an NRHO.
      The three-body machinery of Section~\ref{sec:cr3bp} is the right framework
      there, which is the real reason that part exists.
\item \textbf{$J_2$ is the right first-order term here and only here.} Over the
      $\mTmissionH\unit{h}$ of this mission it dominates. Over ten days it does
      not: the GRAIL-derived field and the $C_{22}$ ellipticity take over, and
      low lunar orbits become genuinely unstable. Nothing in this report should
      be extrapolated to a multi-day loiter.
\item \textbf{The injection dispersion used here is optimistic.} Apollo lunar
      module insertion errors were several metres per second, not the
      $\mDvZero\unit{m/s}$ drawn in Section~\ref{sec:proximity}. The
      $\mDVprox\unit{m/s}$ proximity budget is therefore a lower bound, and the
      Monte Carlo of Table~\ref{tab:summary} widens the spread without changing
      the law it samples from.
\item \textbf{The arm result is static.} A 5R manipulator on a free-flying base
      is an 11-degree-of-freedom system; holding the base fixed is an attitude
      control assumption, not a mechanical fact. The $\mTauMax\unit{N\,m}$ peak
      is a static load. A capture executed at speed would add inertial coupling
      of the same order for a $\mArmMass\unit{kg}$ arm.
\end{itemize}
